...
def vertical_gradient():
    ...
    increment_color = tuple((end - start) / abs(y_distance) for end,
                            start in zip(end_color, start_color))
    ...
    for i in range(abs(y_distance)):
        color = (int(i*inc)+start for inc,
                 start in zip(increment_color, start_color))
